...
# P-body proteins
gg.pb =  plotCompareProfile(rbp.raw.exp.gat,total.exp.gat,pbody,"P-body",cl="Class",gp="UniprotID",print2pdf=T,print2jpg=F)+geom_line(data=gg.pb$data[which(gg.pb$data$GeneName == "EDC3" | gg.pb$data$GeneName == "EDC4"| gg.pb$data$GeneName == "XRN1"),],aes(x=Fraction,y=Abundance,color=GeneName))+scale_color_manual(values = c("#A3A500", "#E76BF3","#F8766D", "#00BFC4","#00BF7D"))
...
```{r Stress-granule-clusters}
head(msgp)

msgp.dat = exprs(rbps.res)[which(rownames(exprs(rbps.res)) %in% msgp$UniprotID),]
rownames(msgp.dat) = msgp$GeneName[which(msgp$UniprotID %in% rownames(msgp.dat))]

h = hclust(dist(msgp.dat))
...
![Nucleoplasm proteins in RBP and Total protein capture](Plots/Nucleoplasm.pdf)  

![PolyA binding proteins in RBP and Total protein capture](Plots/PABP.pdf)  
...
__*PolyA binding proteins*__  
There were 4 polyA binding proteins in Uniprot - PABP1, PABP3, PABP4 and PABP1L of which only PABP1 and PABP4 were present in the RBP dataset. PABPC1 binds the 3', polyA tail of mRNA and is known to shuttle between the nucleus and cytoplasm.PABP-C1 stimulates the translation of cellular polyadenylated mRNAs by enhancing 40S ribosome subunit recruitment and 60S subunit joining. In the total protein data (no-cross link), we expect to see these proteins wherever translation happens/ribosomes hang out - in the cytoplasm and ER 
